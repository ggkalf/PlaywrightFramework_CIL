...
test('Test participation history page', async ({ page }) => {
    ...
    const buffer = fs.readFileSync("rel39_files/users_qa02.txt");
    ...
    await page.goto('account/profile/games/dbg-participation-history/details/62361');
    ...
    expect(name2).toBe('Your line:')

    // const name2 = await page.innerText('#il-web-app > div.exc-container.exc-container__body.exc-container--with-bottom-margin.account-container > div > section > div > div.account-page.account-page--no-bottom-gap > section.ticket-receipt.participation-history > article > header > div.ticket-receipt__header-info > div.ticket-receipt__game-name');
    // expect(name2).toBe('Mega Millions')
